# Feature: Separate IssueRelation API from IssueUpdate in [linear-query.sh](<http://linear-query.sh>)
...
* `create-relation` arg shape: `--type <duplicate|blocks|related> --issue <issue-uuid> --related <related-uuid>`. Output: `{id, type}` on success. Error class: `_die 2` for input validation, `_die 3` for GraphQL errors (per existing [linear-query.sh](<http://linear-query.sh>) convention).
...
* `save-issue --duplicate-of` two-step flow:
  ...
  2. **REMOVE** the `input=$(... '. + {duplicateOf:$v}')` line that appends to IssueUpdateInput. (Substrate bug — line \~536 of current [linear-query.sh](<http://linear-query.sh>).)
