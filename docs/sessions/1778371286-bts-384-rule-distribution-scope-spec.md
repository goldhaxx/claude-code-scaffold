...
- [ ] **AC-4: hub-only filter at sync.** Files with `scope: hub-only` never appear in any downstream `pull-plan`, regardless of node `role`. The hub's own self-pull (`role: hub-substrate-developer`) does include them.
