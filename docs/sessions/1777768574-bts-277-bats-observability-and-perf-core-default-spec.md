...
* **Manifest update.** [Bats-report.sh](<http://Bats-report.sh>)'s manifest gains a new `side-effect: writes-bats-runs-jsonl` declaration and any new failure-modes; the new perf-core branch is a refinement of the existing parallel-jobs path (no new caller/depends-on changes expected — verify via drift-guard).
