...
### Step 2 (GREEN): Add `cmd_create_relation` to [linear-query.sh](<http://linear-query.sh>)
...
* **Test:** Add a static assertion to `issue-relation.bats`: [linear-query.sh](<http://linear-query.sh>) contains `cmd_create_relation` AND does NOT contain the broken `duplicateOf:$v` line in `cmd_save_issue`.
...
- [ ] `cmd_create_relation` exists in [linear-query.sh](<http://linear-query.sh>) + has dispatcher entry + help text.
