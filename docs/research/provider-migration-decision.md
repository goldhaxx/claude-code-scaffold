# Provider Migration Decision Reference

> Tier 2 reference (BTS-387). Excluded from Claude Code auto-load; read on-demand by agent or operator following the provider-integration rule's `anchors.evidence` pointer.

This content was extracted verbatim from `.claude/rules/provider-integration.md` during the BTS-387 atomization audit. The atom file retains the directive + how-to-apply summary; this reference holds the full http-vs-MCP tradeoff matrix, BTS-183 dead-code sweep context, and the OVERRIDE-pattern stubbing convention.

## The Rule (full statement)
