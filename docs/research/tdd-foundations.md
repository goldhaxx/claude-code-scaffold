# TDD Foundations Reference

> Tier 2 reference (BTS-387). Excluded from Claude Code auto-load; read on-demand by agent or operator following the tdd rule's `anchors.evidence` pointer.

This content was extracted verbatim from `.claude/rules/tdd.md` during the BTS-387 atomization audit. The atom file retains the R-G-R cycle directive, Live-API gate, and when-tests-break protocol. This reference holds the expanded rationale, BTS-115/170 incidents, test-structure conventions, what-to-test heuristic, hooks-integration mechanism, strict-mode-bats discipline (BTS-127), bats-report.sh tooling reference (BTS-118/137), and test-execution discipline (BTS-383).

## The Red-Green-Refactor Cycle (full)
...
After every file edit, the test suite runs automatically via hooks. If tests fail after your change, fix immediately before proceeding.
