...
7. If any step adds, removes, or modifies preset infrastructure (commands, rules, agents, skills, hooks, scripts, or sync behavior), add a final step to update documentation. Read these files only when this step applies:
   - **Hub-wide changes** (modifying hub-tracked files): update the relevant file in `.ccanvil/guide/` (hub section, above `<!-- NODE-SPECIFIC-START -->`). If conventions or "do not" rules changed, update the hub section of `CLAUDE.md` (below `<!-- HUB-MANAGED-START -->`).
